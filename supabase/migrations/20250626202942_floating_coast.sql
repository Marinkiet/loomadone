/*
  ...
  1. Changes
    - Modify the progression system to require 10 completed interactive games before unlocking the next topic
    - Update the topic progress calculation to be based on completed games (10 games = 100%)
    - Ensure topics are automatically marked as completed when 10 games are finished
    - Implement automatic unlocking of the next topic in sequence

  2. Functions
    - `has_completed_required_games`: Checks if a user has completed at least 10 games for a topic
    - `get_next_topic`: Finds the next locked topic in sequence
    - `update_topic_progress_and_unlock`: Updates progress and unlocks next topic when criteria met
    - `update_topic_progress_after_game`: Updates topic progress after each game session
*/
...
DROP TRIGGER IF EXISTS update_topic_progress_after_game_trigger ON user_game_sessions;
...
DROP TRIGGER IF EXISTS update_topic_progress_trigger ON user_topic_progress;
